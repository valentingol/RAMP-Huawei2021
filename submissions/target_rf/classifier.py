...
class Classifier:
    def __init__(self):
        ...
        self.timestamp = 96
        self.n_features = 64
        self.iterative_pedictions = 5
        self.unsupervised_tresholds = [0.1, 0.6]
    ...
    def fit(self, X_source, X_source_bkg, X_target, X_target_unlabeled,
        ...
        # fine tune on target:
        self.full_timestamp = X_target.shape[1]
        batches = self.make_batches_train(X_target, y_target, val_prop=0.2)
        self.train_nn(batches)
        self.train_gb(batches)
        # iterative prediction to labelised more data
        for _ in range(self.iterative_pedictions):
            preds = self.predict_proba(self, X_target_unlabeled, X_target_bkg)
